...
resource "yandex_compute_instance" "psql" {
  name        = "psql" #Имя ВМ в облачной консоли
  hostname    = "psql" #формирует FDQN имя хоста, без hostname будет сгенрировано случаное имя.
  platform_id = "standard-v3"
  zone        = "ru-central1-a" #зона ВМ должна совпадать с зоной subnet!!!
...
resource "local_file" "inventory" {
  ...
  [db]
  ${yandex_compute_instance.psql.network_interface.0.ip_address}
